...
impl GraphLinks {
    ...
    /// resulting `GraphLinks` via [`GraphLinksEnum::MmapRanged`], exactly as a
    /// borrowable universal-IO handle owns its whole-file mapping — the
    /// self_cell encapsulates the borrow of the view over the sub-slice.
    ///
