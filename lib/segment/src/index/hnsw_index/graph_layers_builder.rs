...
///
/// The gap is `Geometric(1 - q)`, so this draws once per *kept* edge plus one
/// overshooting draw per node, instead of once per enumerated edge.
#[derive(Clone, Copy)]
...
impl DropGapSampler {
    fn new(q: f32) -> Self {
        // Every other out-of-range `q` has a decided answer in `sample`, but a
        // NaN compares false against both bounds and would fall through to a NaN
        // gap - which `as usize` saturates to 0, "keep every edge" by accident
        // rather than by decision. Normalise it to the value that already means
        // that. The build call site cannot produce a NaN (`1 - 2/links` is at
        // worst `-inf`), but `subgraph_connectivity` is public.
    ...
    /// injected by tests. Saturates rather than wrapping: `u` near 1 gives 0,
    /// and `q` near 1 gives a gap past the end of any link slice.
    fn gap_from_uniform(u: f64, inv_ln_q: f64) -> usize {
        // `sample` establishes this; the tests inject the two ends of it.
        debug_assert!(u > 0.0 && u <= 1.0, "u={u} is outside (0, 1]");
...
impl GraphLayersBuilder {
        ...
        loop {
            ...
            for current_layer in (0..=entry_layer).rev() {
                ...
                while let Some(current_point) = queue.pop() {
                    ...
                    // Charging the whole slice up front is exactly what the
                    // per-edge `+= 1` used to add: the old loop had no early
                    // exit, and `spent_budget` is only read once every layer
                    // has been walked.
    ...
    pub fn merge_block(&self, block: GraphLayersBuilder, to_global: &[PointOffsetType]) -> usize {
        assert_eq!(
            ...
            to_global.len(),
            "payload block holds {} points for {} mapped ids",
            block.links_layers.len(),
            to_global.len(),
        );
        ...
        for (local_id, levels) in block.links_layers.into_iter().enumerate() {
            // The loop below writes into `self.links_layers[global_id][level]`, so
            // a block point that somehow carried a level 1 would either panic on
            // a target point without one or, worse, append level-0 links into a
            // target's level 1. Structurally impossible - the block path never
            // calls `set_levels` - and O(1) to hold to.
            assert_eq!(levels.len(), 1, "payload block points only have level 0");
...
mod tests {
    ...
    fn test_drop_gap_sampler_degenerate_probabilities() {
        ...
        for _ in 0..100 {
            ...
            // Neither bound in `sample` catches a NaN, so `new` folds it into
            // the keep-everything case rather than let it reach the sampling
            // branch and land there by cast saturation.
            assert_eq!(DropGapSampler::new(f32::NAN).sample(&mut rng), 0);
        }
    }
